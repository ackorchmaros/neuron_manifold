...
# source: https://github.com/PBrdng/LearningAlgebraicVarieties/blob/master/src/estimate_dimension.jl
...
"""
function DimensionDiagramsSaveFig(
    data::Array{T,2},
    projective::Bool,
    fout::String;
    ...
    dimension_estimates = map(diagrams) do m
        d = EstimateDimension(m, data, ϵ, projective,fout)
        Plots.plot!(p, ϵ, d, label = string(m), lw = lw, linecolor = colors["$m"]) 
        d
    end
    println("The estimated dimensions are ", vcat(dimension_estimates...)) 
    ...
    y_upper = min(y_upper, maximum(vcat(dimension_estimates...)))
...
# Wrapper #
#################
function EstimateDimension(method::Symbol, data::Array{T,2}, ϵ::Vector{S}, projective::Bool, fout::String) where {S,T<:Number}
    if method == :CorrSum
        return EstimateDimensionCorrSum(data, ϵ, projective, fout)
    elseif method == :BoxCounting
        return EstimateDimensionBoxCounting(data, ϵ, projective, fout)
    elseif method == :NPCA
        return EstimateDimensionNPCA(data, ϵ, projective, fout)
    elseif method == :MLE
        return EstimateDimensionMLE(data, ϵ, projective, fout) 
    elseif method == :ANOVA
        return EstimateDimensionANOVA(data, ϵ, projective, fout)
    else
...
# PCA #
#################
function EstimateDimensionPCA(data::Array{T,2}, projective::Bool, fout::String) where {T <: Number}
    if !projective
        n = size(data, 1)
...
# Nonlinear PCA #
#################
function EstimateDimensionNPCA(data::Array{T,2}, ϵ_array::Vector{S}, projective::Bool, fout::String) where {S,T<:Number}
...
function EstimateDimensionNPCA(data::Array{T,2}, projective::Bool; eps_ticks = 25,fout::String) where {T<:Number}
    ϵ = Array{Float64}(range(0.1, length =eps_ticks, stop = 0.9))
    EstimateDimensionNPCA(data, ϵ, projective,fout)
end
...
#########################
function EstimateDimensionCorrSum(data::Array{T,2}, ϵ_array::Vector{S}, projective::Bool, fout::String) where {S,T<:Number}
    n = size(data, 1)
    m = size(data, 2)
    if !projective
        dist = ScaledEuclidean(data)
...
function EstimateDimensionCorrSum(data::Array{T,2}, projective::Bool; eps_ticks = 25,fout::String) where {T<:Number}
    ϵ = Array{Float64}(range(0.1, length = eps_ticks, stop = 0.9))
    EstimateDimensionCorrSum(data, ϵ, projective, fout)
end

##########################
# Box Counting Dimension #
##########################
function EstimateDimensionBoxCounting(data::Array{T,2}, ϵ_array::Vector{S}, projective::Bool, fout::String) where {S,T<:Number}
    n = size(data, 1)
    m = size(data, 2)
...
function EstimateDimensionBoxCounting(data::Array{T,2}, projective::Bool; eps_ticks = 25, fout::String) where {T<:Number}
    ϵ = Array{Float64}(range(0.1, length = 25, stop = 0.9))
    EstimateDimensionBoxCounting(data, ϵ, projective, fout)
end
...
###########################
function BL_MLE(dist::Array{T,1}, ϵ::Number, projective::Bool, fout::String) where {T<:Number}
    k = length(dist)
    if !projective
...
function EstimateDimensionMLE(data::Array{T,2}, ϵ_array::Vector{S}, projective::Bool, fout::String) where {S,T<:Number}
    m = size(data, 2)
    if !projective
        dist = ScaledEuclidean(data)
    else
    ...
    estimators = map(ϵ_array) do ϵ
            ...
            for i = 1:m
                ...
                k = length(index)
                if k > 1
                    estimator[i,:] = [BL_MLE(dist_i[index], ϵ, projective,fout) * k, k]
                else
                    estimator[i,:] = [0.0, 0]
                end
            end
...
function EstimateDimensionMLE(data::Array{T,2}, projective::Bool; eps_ticks = 25,fout::String) where {T<:Number}
    ϵ = Array{Float64}(range(0.1, length = 25, stop = 0.9))
    EstimateDimensionMLE(data, ϵ, projective,fout)
end
...
function DQV_Estimator(dist::Array{T,2}, fout::String) where {T<:Number}
...
function EstimateDimensionANOVA(data::Array{T,2}, ϵ_array::Vector{U}, projective,fout::String) where {T,U <: Number}
    n = size(data, 1)
    m = size(data, 2)
    dist = zeros(T, m, m)
    D = zeros(Float64, m, m)
...
function EstimateDimensionANOVA(data::Array{T,2}, projective::Bool; eps_ticks = 25,fout::String) where {T<:Number}
    ϵ = Array{Float64}(range(0.1, length = 25, stop = 0.9))
    EstimateDimensionANOVA(data, ϵ, projective,fout)
end
