# Correlation dimension with geodesic distance and Breiding estimation 

# author:Annachiara Korchmaros
# method: the estimation is given by eq (10) in "Learning algebraic varieties from samples"
...
#(6) compute dimension estimate
print('Computing dimension')
increment=(1-0.1)/checks
r_range=np.arange(0.1, 1,increment) 
r_range_list=r_range.tolist()

output=[C(i,data) for i in r_range_list[0:(len(r_range_list) +1)]]
output_dif=np.diff(output)

epsilon_log=[math.log(i) for i in r_range_list[0:(len(r_range_list) +1)]]
epsilon_dif=np.diff(epsilon_log)

D=abs(output_dif/epsilon_dif)
# save D
d=[]
d.append(D)
np.save(fout,d)
